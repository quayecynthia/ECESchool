...
public class EcoleDAO extends DAO<Ecole>{
    ...
    public boolean createNiveau(String nom) throws SQLException{
        boolean success = false;
        boolean found = false;
        
        Statement stmtt = connect.createStatement();
        ResultSet rsett = stmtt.executeQuery("SELECT * FROM niveau WHERE nom LIKE '"+nom+"'");
        
        while(rsett.next()){
            found = true;
        }
        if(!found){
            int ligne = stmtt.executeUpdate("INSERT INTO niveau (id, nom) VALUES (NULL, '"+nom+"')");
            
            if(ligne == 1)success = true;
            
        }
        else System.out.println("Le niveau existe déjà");
        
        return success;
    }

    public boolean deleteDis(String nom) throws SQLException{
        boolean success = false;
        boolean found = false;
        ...
        if(found){
            int ligne2 = stmtt.executeUpdate("DELETE FROM discipline WHERE nom LIKE '"+nom+"'"); 
    ...
    public boolean deleteNiveau(String nom) throws SQLException{
        boolean success = false;
        boolean found = false;
        ...
        if(found){
            int ligne2 = stmtt.executeUpdate("DELETE FROM niveau WHERE nom LIKE '"+nom+"'"); 
...
//                + "(SELECT classe.id FROM classe WHERE classe.ecole_id LIKE "+ ec.getId()+ " ) )");
            ...
            ResultSet rset2 = stmt2.executeQuery("SELECT nom FROM `discipline`");
