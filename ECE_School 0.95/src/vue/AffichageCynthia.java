 ...
 * @author cynth
 */
public class AffichageCynthia extends JFrame{

    /**
     * Creates new form Affichage
     */
    private static Ecole ecole = new Ecole();
    public AffichageCynthia() {
        initComponents();
    }
    ...
    private void buttonElevesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonElevesActionPerformed
        ...
        Object[] row = new Object[3];
            ...
            int j = ecole.getEleves().get(o).getId();
            if(o >= 0){
                rechercherEleve(j);
            }
            else{
            ...
            int o = tabEleves.getSelectedRow();
            System.out.println("select:"+ecole.getEleves().get(o).getId());
            ...
            for(int i =0; i<ecole.getClasses().size(); i++){
                ...
                for(int j = 0; j<c.getEleves().size(); j++){
                    int id = c.getEleves().get(j).getId();
                    System.out.println("Id: "+id);
                    if( id == ecole.getEleves().get(o).getId()){
                        System.out.println("Boucle: "+ecole.getEleves().get(o).getId());
                        if(o >= 0){
                            nouveauBulletin(id);
                            trouve = true;
                        }
    ...
    private void nouveauBulletin(int id){
        ...
        buttonBulletin.addActionListener(new ActionListener(){
            ...
            //detailsBulletinN(trimestre, iDEleve,"");
            rechercherEleve(id);
    ...
    private void afficherBulletinN(int b, int id, Eleve eleve){
        ...
        for(int i = 0; i<ecole.getProfs().size(); i++){
            String c = ecole.getProfs().get(i).getNom();
            fieldProf.addItem(c);
        }
        for(int i = 0; i<ecole.getClasses().size(); i++){
            Classe c = ecole.getClasses().get(i);
                        ...
                        if(bu.getNumero()==b){
                            ...
                            public void actionPerformed(ActionEvent e){
                                ...
                                if(!"".equals(fieldApp.getText()) && !"Selectionner la discipline".equals(fieldDiscipline.getSelectedItem()) && !"Selectionner l'enseignant".equals(fieldProf.getSelectedItem())){
                                    ...
                                    System.out.println(app+discipline+prof);
                                    row[0] = discipline;
                                    row[1] = prof;
                                    row[3] = app;
                                    model.addRow(row);
                                            ...
                                            public void actionPerformed(ActionEvent e){
                                                if(!newNote.getText().equals("")){
                                                    ...
                                                    try {
                                                        ...
                                                        if(evalDao.create(evl, dbu.getId())){
            ...
            addClasseProf.addActionListener(new ActionListener(){
                ...
                public void actionPerformed(ActionEvent e){
                    if(!"Selectionner une classe".equals(comboClassesProf.getSelectedItem())){
                        String niveau = comboClassesProf.getSelectedItem().toString();
                        row[0] = niveau;
                    model.addRow(row);
...
private void rechercherClasse(int id){
    ...
    private void updateEleveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateEleveActionPerformed
        int i = tabEleves.getSelectedRow();
        DefaultTableModel model = (DefaultTableModel)tabEleves.getModel();
        if(i>=0){
            model.setValueAt(classeEleve.getSelectedItem(), i,2);
        }
        else{
            JOptionPane.showMessageDialog(null, "Selectionnez un élève !");
        }
    }//GEN-LAST:event_updateEleveActionPerformed
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AffichageCynthia().setVisible(true);
            }
        });
    }
